...
echo "> Finished removing prefix from environment variables."

# Injects the configuration file into the HTML file
sed -i -e "/RUN-TIME CONFIGURATION/r ${CONFIG_FILE}" ${HTML_FILE}

echo "> Finished injecting configuration file into HTML file."

# Removes the temporary configuration file
rm $CONFIG_FILE

echo "> Finished removing temporary configuration file."
